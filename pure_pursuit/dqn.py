...
if __name__ == "__main__":
    ...
    assert isinstance(envs.single_action_space, gym.spaces.Discrete) or isinstance(envs.single_action_space, gym.spaces.MultiDiscrete), "only discrete action space is supported"
    ...
    for global_step in range(args.total_timesteps):
        ...
        if random.random() < epsilon:
            ...
            q_values = q_network(torch.Tensor(obs[:, :, 0]).to(device))
        ...
        if global_step > args.learning_starts:
            if global_step % args.train_frequency == 0:
                ...
                old_val = q_network(data.observations[:, :, 0]).gather(1, data.actions).squeeze()
